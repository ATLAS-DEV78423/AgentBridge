...
import { BUNDLE_SCHEMA_VERSION, Bundle, BundleExportOptions } from './types.js';
...
function redactSecrets(bundle: AgentBundle): AgentBundle {
  ...
  for (const section of ['instructions', 'skills', 'commands', 'agents', 'mcpServers', 'permissions', 'hooks', 'opaque'] as const) {
    for (const resource of redacted[section]) {
      if (resource.content && containsSecret(resource.content)) {
        resource.content = '[REDACTED]';
      }
      if (resource.metadata) {
        for (const [key, value] of Object.entries(resource.metadata)) {
          if (typeof value === 'string' && containsSecret(value)) {
            resource.metadata[key] = '[REDACTED]';
          }
        }
      }
    }
...
export async function exportBundle(
  ...
  const bundleData: Bundle = {
    manifest: {
      ...
      checksum,
      resourceCount: countResources(processedBundle)
    },
...
function countResources(bundle: AgentBundle): number {
  return bundle.instructions.length +
